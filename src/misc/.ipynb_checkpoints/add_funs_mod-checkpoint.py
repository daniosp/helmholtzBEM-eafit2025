...
def eval_sol(ev_coords, coords, elems, u_boundary, q_boundary, k, domain_type):
    ...
    for pt in range(npts):
        for ev_cont, elem in enumerate(elems):        
            ...
            # Test if the evaluation point is colinear with the element j
            EP_j = coords[elem[0]] # End Point j
            EP_j_1 = coords[elem[1]] # End Point j+1

            j_dist = EP_j_1 - EP_j
            L_j = norm(EP_j_1 - EP_j) # Length of the j-th element
            j_dir = j_dist/L_j

            vec_to_point = pt_col - EP_j
            cross = j_dir[0] * vec_to_point[1] - j_dir[1] * vec_to_point[0]
            

            if abs(cross) < 1e-6: # Evaluation point is colinear
